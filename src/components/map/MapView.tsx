...
import { Compass, LocateFixed, Copy, Twitter, X } from 'lucide-react';
import { toast } from 'sonner';

mapboxgl.accessToken = process.env.NEXT_PUBLIC_MAPBOX_TOKEN || '';

// ── Device capability check ──────────────────────────────────────────────
// Terrain + 3D buildings + antialiasing stacked on top of a globe projection
// is heavy on mid/low-end Android GPUs. We degrade gracefully instead of
// forcing the full "kitchen sink" render on every device.
...
export function MapView({ locations, client, media }: MapViewProps) {
  ...
  // Callback when flight completes or is skipped
  const handleFlightComplete = useCallback((isSkipped?: boolean) => {
    if (isSkipped) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Add 3D Terrain — skip entirely on low-power devices, and even on
      // capable devices this was previously active from zoom 1.5 (globe view)
      // where elevation is imperceptible. Exaggeration lowered from 1.5 to 1.2.
      ...
      // Add 3D Buildings — already gated to minzoom 14 (good), but skip the
      // layer entirely on low-power devices since fill-extrusion recompute
      // during a pitched flyTo/orbit is the most expensive per-frame cost.
      if (!isLowPowerRef.current) {
    ...
    map.on('moveend', () => {
      ...
      const [tlng, tlat] = targetCenterRef.current;
      const latDist = Math.abs(c.lat - tlat);
      const lngDist = Math.abs(c.lng - tlng);
      const needsReset = Math.abs(b) > 5
        || Math.abs(p - (isJourneyStartedRef.current ? 45 : 0)) > 5
        || latDist > 0.15
        || lngDist > 0.15;
  ...
  // ── Track target center for recenter button ─────────────────────────────────
  useEffect(() => {
    if (isJourneyStarted && locations.length > 0) {
      const loc = locations[currentJourneyIndex];
      targetCenterRef.current = [loc.longitude, loc.latitude];
    } else if (selectedLocation) {
      targetCenterRef.current = [selectedLocation.longitude, selectedLocation.latitude];
    } else if (locations.length > 0) {
      targetCenterRef.current = [locations[0].longitude, locations[0].latitude];
    }
  }, [currentJourneyIndex, selectedLocation, isJourneyStarted, locations]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentJourneyIndex, isJourneyStarted, prefersReducedMotion]);
  ...
  return (
    <div className="relative bg-[#0a0a0f]" style={{ width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      {/* ── Recenter button (appears when user moves away from location) ─────── */}
      <AnimatePresence>
        {showReset && !showIntro && (
          <motion.div
            ...
            className="fixed left-6 bottom-[calc(9%+env(safe-area-inset-bottom))] md:left-8 md:top-1/2 md:-translate-y-1/2 z-[40]"
          >
            <motion.button
              ...
              onClick={() => {
                ...
                const [lng, lat] = targetCenterRef.current;
                const sigs = [
                  { zoom: 13.5, pitch: 58 },
                  { zoom: 12.8, pitch: 65 },
                  { zoom: 14.0, pitch: 48 },
      ...
      {/* ── Journey player controls (hidden while panel is open) ──────────── */}
        <JourneyPlayer
          locations={locations}
          mapRef={mapRef}
          isStarted={isJourneyStarted && !isPanelOpen}
