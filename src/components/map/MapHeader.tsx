...
export function MapHeader({ 
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[40] flex items-center justify-between px-4 md:px-6 py-3 md:py-5 bg-gradient-to-b from-black/30 to-transparent pointer-events-none">
      ...
      <div className="flex items-center gap-1.5 md:gap-4 pointer-events-auto flex-shrink-0">
        <AnimatePresence>
          {showExplore && (
            <motion.button
              ...
              onClick={onExplore}
              className="flex items-center gap-1.5 md:gap-2 p-2 sm:px-5 sm:py-2 md:py-2.5 bg-white/10 hover:bg-white/20 border border-white/15 text-white rounded-full transition-all group"
            >
              <Library className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#f5c542]" />
              <span className="hidden sm:inline text-xs md:text-sm font-medium uppercase tracking-[0.1em]">Explore</span>
            </motion.button>
          )}
        </AnimatePresence>
