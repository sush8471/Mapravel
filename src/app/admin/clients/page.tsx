...
import { listClients, listAllLocations } from '@/lib/admin-api';
...
export default function ClientsPage() {
  ...
  const [locations, setLocations] = useState<Pick<Location, 'id' | 'client_id'>[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Page views — graceful fallback if table doesn't exist
      try {
        const viewsRes = await supabase.from('page_views').select('*', { count: 'exact', head: true });
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Stats Cards */}
      <div className="flex md:grid md:grid-cols-3 gap-6 overflow-x-auto pb-4 md:pb-0 scrollbar-none snap-x snap-mandatory">
        {loading ? (
          [1, 2, 3].map(i => (
            <div key={i} className="h-32 bg-[#111116] border border-white/5 rounded-2xl animate-pulse min-w-[280px] flex-shrink-0 md:min-w-0 md:flex-shrink"></div>
          ))
        ) : (
          stats.map((stat, i) => (
            <div 
              key={i} 
              className="bg-[#111116] border border-white/5 rounded-2xl p-6 flex items-center gap-5 group hover:border-white/10 transition-colors min-w-[280px] flex-shrink-0 md:min-w-0 md:flex-shrink snap-start"
            >
