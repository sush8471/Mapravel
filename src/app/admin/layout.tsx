...
export default function AdminLayout({
  ...
  const handleLogout = async () => {
    // Clear the admin-token cookie by making a request, or just delete it client-side
    document.cookie = 'admin-token=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT';
    router.push('/admin/login');
  };

  // Handle Login page separately — no sidebar
  if (pathname === '/admin/login') {
    return <div className="min-h-screen bg-[#0a0a0f]">{children}</div>;
  }

  // Sidebar Layout (middleware guarantees we're authenticated here)
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex font-sans text-white">
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <span className="text-[#f5c542] font-bold text-lg">Mapravel</span>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <div id="admin-scroll-container" className="flex-1 overflow-y-auto p-6 md:p-10 pt-24 md:pt-10">
